...
```
```{r,echo=FALSE,fig.show='hold',fig.width=10,fig.height=6}
par(mfrow=c(1,2))
...
`weightmat`
: Dissimilarity weights ($z_{ij}$ and $w_{ij}$) can be set as `weightmat`. This must be a matrix of the same dimensions as the dissimilarity argument `delta`. Let's say we found out that there was a study error where comparing cousins with Aunt was messed up, so we want ot ignore that dissimilarity .
...
* `dmax` is an important parameter to bring out clusteredness. While the default value for `dmax` works well in many cases, it is nevertheless a good idea to try out different values. One strategy that we found useful (particularly with COPS-C) is to fit the standard MDS and then investigating the reachability plot of the cordillera. If `dmax` is too high, this is typically apparent (and can happen easily if there is very little clusteredness in the plot and the whole data set is considered by OPTICS to be essentially one cluster).  Similarly, it pays out to investigate the reachability plot for a fitted COPS model (`plot(...,plot.type="reachplot")`) and see if the `dmax` was set too large (or not large enough). Here's an example for the `GOPdtm` data from package `smacof`.
```{r,fig=TRUE}
data(GOPdtm)
dis <- dist(t(GOPdtm))
