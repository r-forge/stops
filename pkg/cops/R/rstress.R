spp <- function (dhat, confdiss, weightmat) 
{
    resmat <- as.matrix(wgths) * as.matrix(dhat - confdiss)^2
    diag(resmat) <- NA
...
rStressMin <- function (delta, r=0.5, type=c("ratio","interval","ordinal"), ties="primary", weightmat=1-diag(nrow(delta)), init=NULL, ndim = 2, acc= 1e-6, itmax = 10000, verbose = FALSE, principal=FALSE) {
  ...
  #  trans <- "mspline"
   ...
   ##first optimal scaling
    ...
    out <- list(delta=deltaorig, dhat=delta, confdist=dout, iord=dhats2$iord.prim, conf = xnew, stress=sqrt(snew), spp=spp,  ndim=p, weightmat=weightmat, resmat=resmat, rss=rss, init=xstart, model="r-stress SMACOF", niter = itel, nobj = dim(xnew)[1], type = type, call=match.call(), stress.m=snew, alpha = anew, sigma = snew, tdelta=deltaold, parameters=c(r=r), pars=c(r=r), theta=c(r=r))
    class(out) <- c("smacofP","smacofB","smacof")
    out
  }
