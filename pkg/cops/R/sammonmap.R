...
#'
#' An implementation to minimize sammon stress by majorization with ratio, interval and ordinal optimal scaling. Uses a repeat loop.
#' 
...
#' res<-sammonmap(as.matrix(dis),type="ordinal",itmax=1000)
#' res
#' summary(res)
#' plot(res)
#' 
#' @export
sammonmap <- function (delta, type=c("ratio","interval","ordinal"),ties="primary", weightmat=1-diag(nrow(delta)), init=NULL, ndim = 2, acc= 1e-6, itmax = 10000, verbose = FALSE) {
  ...
  #  trans <- "mspline"
   ...
   ##first optimal scaling
    ...
    weightmat <- weightmato/delta #sammon weighting
    weightmat[!is.finite(weightmat)] <- 1
