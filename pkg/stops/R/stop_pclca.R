...
#' STOPS version of power curvilinear component analysis for fixed tau
#'
#' Power stress with free kappa, lambda, rho and tau.
#'
...
stop_pclca <- function(dis,theta=c(1,1,1,100),type="ratio",weightmat=1-diag(nrow(dis)),init=NULL,ndim=2,itmaxi=10000,...,stressweight=1,structures=c("cclusteredness","clinearity","cdependence","cmanifoldness","cassociation","cnonmonotonicity","cfunctionality","ccomplexity","cfaithfulness","cregularity","chierarchy","cconvexity","cstriatedness","coutlying","cskinniness","csparsity","cstringiness","cclumpiness","cinequality"), strucweight=rep(1/length(structures),length(structures)),strucpars,verbose=0,stoptype=c("additive","multiplicative")) {
  ...
  wght <- weightmat
  diag(wght) <- 1
  fit <- smacofx::pclca(delta=dis,kappa=theta[1],lambda=theta[2],nu=theta[3],tau=theta[4],type=type,weightmat=wght,init=init,ndim=ndim,verbose=verbose,itmax=itmaxi,...)
  fit$kappa <- theta[1]
  fit$lambda <- theta[2]
  fit$rho <- theta[3]
  fit$tau <- theta[4]
