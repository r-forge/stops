...
stop_spmds <- function(dis,theta=c(1,1,1,100),type="ratio",weightmat=1-diag(nrow(dis)),init=NULL,ndim=2,itmaxi=10000,acc=1e-8,...,stressweight=1,structures=c("cclusteredness","clinearity","cdependence","cmanifoldness","cassociation","cnonmonotonicity","cfunctionality","ccomplexity","cfaithfulness","cregularity","chierarchy","cconvexity","cstriatedness","coutlying","cskinniness","csparsity","cstringiness","cclumpiness","cinequality"), strucweight=rep(1/length(structures),length(structures)),strucpars,verbose=0,stoptype=c("additive","multiplicative"),registry=struc_reg) {
  ...
  nu <- theta[3]
  tau <- theta[4]
  fit <- smacofx::spmds(delta=dis,kappa=kappa,lambda=lambda,nu=nu,tau=tau,type=type,weightmat=wght,init=init,ndim=ndim,verbose=verbose,itmax=itmaxi,acc=acc,...)
  ncall <- do.call(substitute,list(fit$call,list(kappa=kappa,lambda=lambda,nu=nu,tau=tau,type=type,init=init,ndim=ndim,verbose=verbose,itmax=itmaxi,acc=acc)))
  fit$call <- ncall                
  fit$kappa <- kappa
  fit$lambda <- lambda
  fit$nu <- nu
