...
## # Idea for stops function allow an arbitrary number of indices in a weighted multi-objective optimization way; for this use stoplos
## # write stops_foo where foo is the MDS model of interest
## # TODO: also do this with a pareto approach    
## #'  Calculate the weighted multiobjective loss function used in STOPS
## #'
...
## #' 
## #' @export
## stoploss_R<- function(obj,stressweight=1,structures,strucweight=rep(-1/length(structures),length(structures)),strucpars,stoptype=c("additive","multiplicative"),verbose=0,registry=stops_reg)
##     {
##         if(missing(strucpars)) strucpars <- vector("list", length(structures))
##         stressi <- obj$stress.m #we use stress.m everytime
##         pars <- obj$pars
...
##         {
##           struc[i] <- do.call(R$get_entry(structures[i])$index,c(list(confs),strucpars[[i]]))
##           names(struc)[i]  <- R$get_entry(structures[i])$name  
##         }  
##         if("cclusteredness"%in%structures)
##             {
##               indst <- which(structures=="cclusteredness")  
##               cclusteredness <- do.call(stops::c_clusteredness,c(list(confs),strucpars[[indst]]))
##             }
##          if("cregularity"%in%structures)
##             {
##               indst <- which(structures=="cregularity")  
...
##             {
##                indst <- which(structures=="cfaithfulness")
##                cfaithfulness <- do.call(stops::c_faithfulness,c(list(confs),strucpars[[indst]]))
##             }
##          if("chierarchy"%in%structures)
##             {
##                indst <- which(structures=="chierarchy")
