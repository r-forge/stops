...
In this document we introduce the functionality avalaiable in `stops` for fitting multidimensional scaling (MDS) or proximity scaling (PS) models either with a STOPS or COPS idea or not. We start with a short introduction to PS and the models that we have available. We then expalin fitting of these models with `stops`. Next, we introduce the reader to COPS and STOPS models and show how to fit those. For illustration we use the `smacof::kinshipdelta` data set (Rosenberg, S. & Kim, M. P., 1975) which lists percentages of how often 15 kinship terms were not grouped together by college students. 
...
\end{equation}
There are a number of optimization techniques one can use to solve the problem in (\ref{eq:optim}). Strain losses can often solved analytically. 


### Stress Models 
The first  popular type suported of PS in `stops` is based on the loss function type _stress_ \citep{kruskal1964multidimensional}. This uses some type of Minkowski distance ($p > 0$) as the distance fitted to the points in the configuration, 
\begin{equation}
\label{eq:dist}
...
* Explicitly normalized stress: $w_{ij}=(\sum_{ij}\delta^{*2}_{ij})^{-1}$ 
* Stress-1: $w_{ij}=(\sum_{ij} d^{*2}_{ij}(X))^{-1}$. 
* Sammon stress: $w_{ij}=\delta^{*-1}_{ij}$ 
* Elastic scaling stress: $w_{ij}=\delta^{*-2}_{ij}$. 
* S-stress: $\delta^*_{ij}=\delta_{ij}^2$ and $d^*_{ij}(X)=d^2_{ij}(X)$
* R-stress: $\delta^*_{ij}=\delta_{ij}$ and $d^*_{ij}=d^{2r}_{ij}$
* Powermds: $\delta^*_{ij}=\delta_{ij}^\lambda$ and $d^*_{ij}=d^\kappa_{ij}$,
* Power elastic scaling: $w_{ij}=\delta^{*-2}_{ij}$, $\delta^*_{ij}=\delta_{ij}^\lambda$ and $d^*_{ij}=d^\kappa_{ij}$
* Power sammon mapping: $w_{ij}=\delta^{*-1}_{ij}$, $\delta^*_{ij}=\delta_{ij}^\lambda$ and $d^*_{ij}=d^\kappa_{ij}$ 
* Powerstress (encompassing all previous models): $\delta^*_{ij}=\delta_{ij}^\lambda$, $d^*_{ij}=d^\kappa_{ij}$ and $w_{ij}=w_{ij}^nu$ for arbitrary $w_{ij} (e.g., a function of the \delta_{ij})

For all of these models one can use the function `powerStressMin` which uses majorization \citep{deLeeuw:1977} to find the sloution. The function allows to specify a `kappa`, `lambda` and `nu` argument as well as a `weightmat` (the w_{ij}). Note that if $z_{ij}$ and $w_{ij}$ are used, then `weightmat` must be the combination of both.  

The object returned from `powerStressMin` is of class `smacofP` which extends the `smacof` classes to allow for the power transformations. Apart from that the objects are made so that they have maxmum comaptibility to methods from `smacof`. Accordingly, the following S3 methods are available:
...
A `sammon` Mapping
```{r}
res2<-powerStressMin(dis,kappa=1,lambda=1,weightmat=dis^-1)
res2
```

An `elastic` scaling
```{r}
res3<-powerStressMin(dis,kappa=1,lambda=1,weightmat=dis^-2)
res3
```
...
A `powermds` model 
```{r}
res6<-powerStressMin(dis,kappa=2,lambda=1.5,nu=1)
res6
```
...
A `powerelastic` scaling (s-stress with elastic scaling)
```{r}
...
* Classical scaling: $\delta^*_{ij}=-h(\delta_{ij})$ and $d^*_{ij}=d^_{ij}$ 
* Power strain: $\delta^*_{ij}=-h(\delta_{ij}^\lambda)$, $d^*_{ij}=d^_{ij}$ and $w_{ij}=w_{ij}^nu$ for arbitrary $w_{ij}
...
Let us assume we have $K$ different indices $I_k(X)$ for different types of $c-structuredness$. We can then define \emph{STOPS} as methods that are of the form 
\begin{equation}
...
\end{equation}
(multiplicative STOPS, which can be expressed as aSTOPS by logarithms).

 If we have one $I(X^*)=OC(X)$, the OPTICS cordillera, and the transformations applied are power transformations we have our suggestion of COPS.    
...
* c-faithfulness: How accurate is the neighbourhood of $Delta$ preserved in $D$ ($I_k$ is the overlapping of Chen \& Buja, 2013; not yet implemented)
...
#### Usage 
There is a dedicated function `cops`` for fitting COPS models. It's syntax works pretty much like in `stops` only that the `structure` argument is non-existant. 

`cops(dis,loss="strain",...)`

For the example we have 

```{r}
res<-cops(kinshipdelta)
res
summary(res)
```
...
### Optimization
Since the inner optimization problem in STOPS models is hard and takes long, we developed a metaheuristic for the outer optimization problem that needs typically less calls to the inner minimization than `pso` or `SANN`, albeit without the guarantees of convergence to a global minimum for non-smooth functions. It is an adaptation of the Luus-Jakola random search. It can used with the function `ljoptim` which modeled its output after `optim`. It needs as arguments `` a function to optimize, a `lower` and `upper` box constraint for the search region. By using the argument `adaptive=TRUE` or `FALSE` one can switch between our adaptive version and the original LJ algorithm. Accuracy of the optimization can be controlled with the `maxit` (maximum number of iterations), `accd` (terminates after the length of the search space is below this number ) and `acc` argmuents (temrinates if difference two subsequent function values are below this value).  
