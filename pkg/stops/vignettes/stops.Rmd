...
The main usage of the `stops` package is not in solely fitting the _powerstress_ or _powerstrain_ models and their variants from above but allowing to choose the right transformation to achieve a "structured" MDS result automatically. For this, the MDS loss function is augmented to include a penalty for the type of structure one is aiming for. This combination of an MDS loss with a structure penalty is what we call "structure optimized loss" and the resulting MDS is coined Structured Optimized Proximity Scaling" (or STOPS). The prime example for a STOPS model is "Cluster Optimized Proximity Scaling" (COPS) which selects optimal parameters so that the clusteredness appearance of the configuation is improved. 
...
If we have one $I(X)=OC(X)$, the OPTICS cordillera (Rusch et al. 2015a), and the transformations applied are power transformations we have COPS.    
...
We see that the c-clusteredness is higher as comapred to the mSTOPS result - we have a number of distinct 2 object clusters and they are more spread out and distributed more evenly. The dimensions on the other hand are now farther from being orthogonal. 
