...
This approximation $D^*(X)$ to the matrix $\Delta^*$ is found by defining a fit criterion (the loss function), $\sigma_{MDS}(X)=L(\Delta^*,D^*(X))$, that is used to measure how closely $D^*(X)$ approximates $\Delta^*$.
...
\end{equation}
Here, the $w_{ij}$ and $z_{ij}$ are finite weights, with $z_{ij}=0$ if the entry is missing and $z_{ij}=1$ otherwise. There are a number of optimization techniques one can use to solve this optimization problem. 
...
* **Power MDS** (Buja et al. 2008, Rusch et al. 2021): $\delta^*_{ij}=\delta_{ij}^\lambda$ and $d^*_{ij}=d^\kappa_{ij}$, $w_{ij}=1$
...
* **Powerstress** (encompassing all previous models; Buja et al. 2008, Rusch et al. 2021): $\delta^*_{ij}=\delta_{ij}^\lambda$, $d^*_{ij}=d^\kappa_{ij}$ and $w_{ij}=w_{ij}^\nu$ for arbitrary $w_{ij}$ (e.g., a function of the $\delta_{ij}$)
* **local MDS** (Chen & Buja, 2009):  Let $N_k$ define the symmetric set of nearby pairs of points $(i,j)$ so that $(i,j)\in N_k$ iff $i$ is among the $k-$nearest neighbours of $j$ or the other way round. Then we have a stress model with 
 \begin{equation}
...
\sqrt{w}d_{ij}(X)  &\mbox{ if } (i,j) \in N_k\\
  \end{cases}
\end{equation}
where $\delta_{\infty} \rightarrow \infty$ is a large ``imputed'' dissimilarity that is constant and $w$ a small weight. For simplification, one takes $w \sim 1/\delta_{\infty}$ and expands (\ref{eq:lmds}) to the standard lMDS objective. In the standard lMDS objective one uses the tuning parameter $\tau=2w\delta_{\infty}$ for given $k$.
...
* **Powerstrain** (Buja et al. 2008, Rusch et al. 2021: $\delta^*_{ij}=-h(\delta_{ij}^\lambda)$, $d^*_{ij}=d_{ij}$ and $w_{ij}=w_{ij}^\nu$ for arbitrary $w_{ij}$
...
### Energy Models
The third type of PS supported in `stops` is based on the energy type losses of pairwise attraction and repulsion between objects (Chen & Buja, 2014, Rusch et al 2023). It is related to graph drawing: if vertices are equated with objects, the edge weights are the $\delta^*_{ij}$, the node positions in the layout is the $X$ and the distance between nodes in the layout are $d_{ij}(X)$.
...
\begin{equation}
 \label{eq:energy}
 \sigma_{\text{EM}}(X;\bm\theta) = \sum_{i<j} w^*_{ij} \left(a d^{*\mu}_{ij}(X) -  b \delta^*_{ij}d^*_{ij}(X) \right)
\end{equation}
with $a,b$ being constants and $d^*_{ij}(X)=g_{ij}(d_{ij}(X);\bm\theta)$ and $\delta^*_{ij}=f_{ij}(d_{ij}(X);\bm\theta)$. For $\mu=2, a=1$ and $b=2$ this is a (\ref{eq:stress}) where terms depending solely on $\delta^*_{ij}$ are disregarded for finding $X$. 

Specific instances of energy models that can be fitted in `stops` are: 

* **Box-Cox MDS** (Chen & Buja, 2014). This is a three-parameter family $\bm\theta=(\mu^{(BC)},\lambda^{(BC)},\rho^{(BC)})^\top$:
\begin{equation}
  \label{eq:bcstress}
 \sigma_{\text{BC}}(X;\bm\theta) = \sum_{i<j} \delta_{ij}^{\rho} \left( BC_{\mu+\lambda}(d_{ij}(X)) - \delta_{ij}^{\lambda} BC_{\mu}(d_{ij}(X)) \right)
\end{equation}
...
BC_{\alpha}(d)=\begin{cases}
  ...
  \end{cases}
\end{equation}
For $w_{ij}=\delta_{ij}, \mu=\lambda=1$ this is very similar to the stress-equivalent energy MDS model. Note that here the distance transformation functions in the attraction and repulsion parts need not be equal. $\sigma_{\text{BC}}(X;\bm\theta)$ shares close similarities with stress with power transformations (power stress): They will often lead to a similar configurations for the same parameters (even equivalent if the exponents for repulsion and attraction parts are the same).

We can fit these models with the `bcStressMin` function.
...
The main contribution of the `stops` package is not in solely fitting the flexible MDS models from above ( _powerstress_, _powerstrain_, _lmds_, _bcStress_ and their variants). We saw that these flexible models als have hyperparameters that have a huge impact on how the final configuration looks like. THat begs the question which hyperparameters shoulf be chosen. Often (and as we did so far) tyhey are simply chosne ad hoc.      

The idea behind STOPS is to allowing to automatically select good hyperparameters to achieve a "structured" MDS configuration. This can be useful in a variety of contexts: to explore or generate structures, to restrict the target space, to avoid artefacts, to preserve certain types of structures and so forth. 

For this, an MDS loss function is subjected to nonlinear transformations and is augmented to include penalties for the type of structures one is aiming for. This combination of an MDS loss with a structuredness penalty is what we call "structure optimized loss" (stoploss) and the resulting MDS is coined "Structured Optimized Proximity Scaling" (or STOPS, Rusch, Mair & Hornik, 2023). STOPS model is related to "Cluster Optimized Proximity Scaling" (COPS; Rusch, Mair & Hornik, 2021) which allows to select optimal parameters so that the clusteredness appearance of the configuation is improved (see below). 

### STOPS

Following Rusch, Mair & Hornik (2023) the general idea is that from given observations $\Delta$ we look for a configuration $X$. The $X$ has properties with regards to its structural appearance, which we call _c-structuredness_ for configuration-structuredness. There are different types of c-structuredness people might be interested in (say, how clustered the result is, or that dimensions are orthogonal or if there is some submanifold that the data live on). We developed indices for these types of c-structuredness that capture that essence in the configuration (see Rusch, Mair & Hornik, 2023b, or Rusch, Mair & Hornik, 2021 for a large list of structures and indices).

We have as part of a STOPS model a proximity scaling loss function $\sigma_{MDS}(\cdot)$, a $\Delta$ and an $X$ and some transformation $f_{ij}(\delta_{ij};\theta)$ and $g_{ij}(d_{ij};\theta)$ that are parametrized (with $\theta$ being finite, e.g., a transformation applied to all observations like a power transformation). These transformations achieve a sort of push towards more structure, so different values for $\theta$ will in general lead to different c-structuredness.
...
Here the $v_0,...,v_k$ are weights that determine how the individual parts (MDS loss and c-structuredness indices) are aggregated and $v_0\geq 0$. Note that in this formulation the aggregation is a sum/product, so the weights must be negative if a higher index stands for more structure and we want more of that structure. Alternatively, the `stressweight` can be negative and the `strucweight` positive. 
...
\end{equation} 
where $\vartheta \subseteq \{X,\theta, v_0, \dots, v_k\}$.  Typically $\vartheta$ will be a subset of all possible parameters here (e.g., the weights might be given _a priori_). 

Minimizing stoploss can be quite difficult. In `stops` we use a nested algorithm combining optimization that internally first solves for $X$ given $\theta$, $\arg\min_X \sigma_{MDS}\left(X,\theta\right)$ in an inner loop, and then optimize over $\theta$ with a metaheuristic in an outer loop. The number of iterations of the outer loop can be controlled with `itmax`, the number of iterations of the inner loop with `itmaxps`.   

#### Optimization 

Implemented metaheuristics are simulated annealing (`optimmethod="SANN"`), particle swarm optimization (`optimmethod="pso"`), DIRECT (`optimmethod="DIRECT"`), stochastic global optimization (`optimmethod="stogo"`), COBYLA (`optimmethod="cobyla"`), Controlled Random Search 2 with local mutation (`optimmethod="crs2lm"`), Improved Stochastic Ranking Evolution Strategy (`optimmethod="isres"`), Multi-Level Single-Linkage (`optimmethod="mlsl"`), Nelder-Mead (`optimmethod="neldermead"`), Subplex (`optimmethod="sbplx"`), Hooke-Jeeves Pattern Search (`optimmethod="hjk"`), CMA-ES (`optimmethod="cmaes"`), Bayesian optimization with Gaussian Process priors and Kriging (`optimmethod="Kriging"`), Bayesian optimization with treed Gaussian processes with jump to linear models (`optimmethod="tgp"`) and Adaptive Luus-Jaakola Search (`optimmethod="ALJ"`). Defaults is "ALJ". In our experience `tgp`, `ALJ`, `Kriging` and `pso` usually work well for relatively low values of `itmax` and if a higher number of `itmax` can be afforded, `ALJ` and `pso` is usually good.
...
See Rusch, Mair, Hornik (2021) or Rusch, Mair, Hornik (2023b) for a precise definition of each c-structuredness index.
...
If we have a single $I(X)=OC(X)$, the OPTICS cordillera (Rusch et al. 2015a), and the transformations applied are power transformations and the weights for the $I(X)$ is negative we essentially have P-COPS (see below).
...
* `lmds`: LMDS; Workhorse: `lmds`, Optimization over $\tau$, $k$.
* `bcstress`: Box-Cox MDS; Workhorse: `bcStressMin`, Optimization over $\mu$, $\lambda$, $\nu$.


#### Usage 

The syntax for fitting a `stops` model is rather straightforward. One has to supply the arguments `dis` which is a dissimilarity matrix and `structures` a character vector listing the c-structuredness type that should be used to augment the PS loss (see above for the types of structures and losses). The metaparameters for the structuredness indices should be given via the `strucpars` argument; it should be a list whose elements are again lists corresponding to each structuredness index and listing the parameters (if the default should be used the list element should be set to `NULL`). The PS loss can be chosen with the argument `loss`. The type of aggregation for the multi-objective optimization is specified in `type` and can be one of `additive` or `multiplicative`. As starting value for the $\theta$ one can supply (`theta`). If not this will be a scalar $1$.
...
We first set up the parameters for the c-structuredness indices. For the OPTICS cordillera we use a `dmax=1`, `epsilon=10` and `minpts=2`, for c-linearity we have no parameters (so using `NULL` will work) and for the c-dependence we have a single parameter, `index`, which we set to 2.   

```{r}
strucpars<-list(list(epsilon=10,minpts=2,dmax=1.3), #cordillera
                NULL,     # c-linearity (has no parameters)
                list(index=1) #c-dependence
                ) 
...
The print function gives us information about the model. The selected hyperparameter was $\lambda=\Sexpr{ressm$parameters[2]}$. With that and the given metaparameters of the indices we have values of 0.24for c-clusteredness, 0.03 for c-linearity and 0.27 for c-dependence. Stress-1 (the square root of raw stress) is 0.0692. 

We plot the obtained configuration (the one for $\lambda=1.4$).
...
We see that the c-clusteredness is a bit higher as compared to the mSTOPS result - we have a number of distinct object clusters (with at least minpts=2) and they are more spread out and distributed more evenly. 
...
We use `tgp` as optimization method and restrict the search space to be between $0$ and $5$.

```{r,eval=FALSE}
set.seed(666)
resa<-stops(kinshipdelta,structures=c("cassociation","cmanifoldness","clinearity"),loss="powermds",theta=c(1,1,1),strucpars=list(NULL,NULL,NULL),type="additive",strucweight=c(-0.5,-0.5,0.5),lower=c(0,0,0),upper=c(5,5,5),optimmethod="tgp",itmax=10)
```
...
We see in this model (`resa`) that indeed the c-manifoldness is almost 1 and c-association is high, which says we have a close to perfect non-linear relationship. How does this relationship look like? 
...
It resembles a simplex (the manifold) and since the objects are arranged at the vertices, this is also associated (non-linearly). 

What we can also see is that there are three clear clusters of 4 or more objects, so c-clusteredness should be high (but we didn't select for that). When looking at the OPTICS cordillera here, we find that the OPTICS cordillera with minpts=4 is indeed higher than for the result from above with using `stress` (the `ressa` model).

```{r}
library(cordillera)
c1<-cordillera(resa$fit$conf,minpts=4,epsilon=10,dmax=1.3)
c2<-cordillera(ressa$fit$conf,minpts=4,epsilon=10,dmax=1.3)
c1
c2
```    

This comes from the definition of c-clusteredness (Rusch, Hoornik & Mair, 2019) where more clusters, more spread-out clusters, more evenly distributed clusters and denser clusters all increase c-clusteredness.
...
Note that it may just as well be possible to have a high c-association and no c-clusteredness at all (e.g., points lying equidistant on a smooth non-linear curve), as we did not optimize for the latter. Note also that the models are not necessarily comparable due to different stress functions - the transformation in `powermds` that is optimal with respect to c-clusteredness would be different. 

Indeed one can even optimize for c-clusteredness alone (setting `stressweight=0`) and using it as a "goodness-of-clusteredness" index (i.e., we let the $d_{max}$ vary between the configurations and take the highest attainable nomred cordillera) then we get a projection with c-clusteredness of 0.71.  
 
```{r,echo=FALSE,eval=FALSE}
resa2<-stops(kinshipdelta,structures=c("cclusteredness"),loss="powermds",verbose=0,strucpars=list(epsilon=10,rang=c(0,1.3),minpts=2),type="additive",strucweight=-1,stressweight=0,lower=0,upper=10)
...
```{r,eval=TRUE}
resa2<-stops(kinshipdelta,structures=c("cclusteredness"),loss="powermds",strucpars=list(list(epsilon=10,dmax=NULL,minpts=2)),type="additive",strucweight=-1,stressweight=0,lower=0.1,upper=10)
resa2
```
This can of course be done for an c-structuredness. 

It is also possible to use the `stops` function for finding the hyperparameters that are optimal for minimizing the badness-of-fit for a given transformation class in the the non-augmented models specified in `loss`, by setting the `strucweight` (the weight of any c-structuredness) to 0. Then the function optimizes the MDS loss function only. 

```{r,eval=FALSE}
ressa<-stops(kinshipdelta,structure=c("clinearity"),strucweight=0,loss="stress",verbose=0,upper=8,lower=0)
```

Here the minimum badness-of-fit by subjecting the $delta$ to a power transformation $\delta^*=\delta^\lambda$ is obtained with $\lambda=1.297$ and a stress-1 of $0.069$. This strategy is similar to how the optimal scaling of the dhats works within other MDS methods (e.g. in nonmetric and spline MDS).   

### COPS
A special STOPS model is P-COPS (Rusch et al. 2021). This is also one of the main use cases for STOPS models. Let us write $X(\theta)=\arg\min_X \sigma_{MDS}(X,\theta)$ for the optimal configuration for given transformation parameter $\theta$. Following the outline of STOPS the overall objective function, which we call \emph{cluster optimized loss (coploss)}, is a weighted combination of the $\theta-$parametrized loss function, $\sigma_{MDS}\left(X(\theta),\theta\right)$, and a c-clusteredness measure, the OPTICS cordillera or $OC(X(\theta);\epsilon,k,q)$ to be optimized as a function of $\theta$ or
\begin{equation}
\label{eq:spstress}
...
* Rusch,  T.,  Mair,  P. and Hornik, K. (2020). STOPS: Structure Optimized Proximity Scaling. Discussion Paper Series / Competence Center for Empirical Research Methods, No 2020/1, WU Vienna, Austria. 

* Rusch, T., Mair, P. and Hornik, K. (2021) Cluster Optimized Proximity Scaling. Journal of Computational and Graphical Statistics, 30:4, 1156-1167. 

* Rusch,  T.,  Mair,  P. and Hornik, K. (2023). Structure-based hyperparameter selection with Bayesian optimization in multidimensional scaling. Statistics & Computing, 33:28. 
